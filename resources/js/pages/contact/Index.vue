...
const debouncedSearch = debounce(() => {
  router.get(route('contacts.index'), { search: search.value }, {
    preserveState: true,
    preserveScroll: true,
    replace: true
  });
                ...
                <div class="overflow-x-auto">
                    <table class="w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                            <tr v-for="(item, index) in contacts.data" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                <td class="px-4 py-2 whitespace-nowrap text-sm">{{ contacts.from + index }}</td>
                                <td class="px-4 py-2 whitespace-nowrap text-sm">{{ item.name }}</td>
                                <td class="px-4 py-2 whitespace-nowrap text-sm">{{ item.phone || '-' }}</td>
                ...
                <div class="px-6 py-4 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700">
                    <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                        <div class="flex items-center text-sm text-gray-500 dark:text-gray-400">
                            Showing
                            <span class="font-medium mx-1">{{ contacts.from || 0 }}</span>
                            to
                            <span class="font-medium mx-1">{{ contacts.to || 0 }}</span>
                            of
                            <span class="font-medium mx-1">{{ contacts.total }}</span>
                            results
                        </div>
